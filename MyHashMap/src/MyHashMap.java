...
public class MyHashMap<K extends Comparable<K>, V> implements MyMap<K, V> {
    ...
    public V put(K key, V value) {
        ...
        if(table[index] == null)
        {
            ...
            if(r == false)
            {
                Entry<K,V> another = new Entry<K,V>(key,value);
                another.next = table[index];
                table[index] = another;
                numEntries++;
            }
        }
        if(getLoadFactor() > 0.75 && primeIndex != primes.length-1)
        {
            rehash();
    ...
    @SuppressWarnings("unchecked")
    private void rehash() {
        Entry<K,V>[] anotherT = new Entry[primes[++primeIndex]];
        for(int i = 0; i < getTableSize(); i++)
        {
            if(table[i] != null)
            {
                ...
                do {
                    ...
                    int hash = p.key.hashCode();
                    int index = hash%anotherT.length;
                    Entry<K,V> q = anotherT[index];
                    anotherT[index] = p;
                    anotherT[index].next = q;
                    p = next;
